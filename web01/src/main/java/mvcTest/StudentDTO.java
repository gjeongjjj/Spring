...
public class StudentDTO extends JoDTO {
	// 외부에서 직접 접근 어렵게. 
	// 테이블에 있는 컬럼명과 동일하게 정하기. 
	// 정수는 int로 실수는 double로
	
	// 1) private로 멤버변수 정의
	private int sno; // int가 아니라 byte나 short로 해도됨. 하지만 내부적으로 그냥 int로 해도됨. 
	private String name;
	private int age;
	private int jno;
	private String info;
	...
	// 멤버 변수명의 2번째 알파벳도 대문자로 하지 말 것. !!!!! 
	// cName 안됨. 
	
	// ** 생성자
	// => default 생성자, 모든 값을 초기화하는 생성자. 
	public StudentDTO() {}
	public StudentDTO(String name, int sno) {
		this.name = name;
		this.sno = sno;
		// delete
	}
	
	public StudentDTO(int jno, String name) {
		this.jno = jno;
		this.name = name;
		// delete
	}
...
//		this(sno, name);
	...
	public StudentDTO(int sno, String name, int age, int jno, String info, double point, 
		...
		this.info = info;
		this.point = point;
		// 연습 ㅣ 부모에게 상속받은 필드까지 다 생성자로 초기화해보기 **************************
	}

	// 2) setter/ getter로 접근해야됨. 
	public int getSno() { return this.sno; } // getter
	public void setSno(int sno) { this.sno = sno; } // setter
	// 오른쪽마우스_source_generateGetterAndSetter
	...
	//    객체의 값을 편리하게 확인하기 위해서 사용 
	@Override 
	// (상속) 어딘가에 조상이 있다는 의미.(StudentDTO가 조상) 오버라이드.
	public String toString() {
		return "StudentDTO [sno=" + sno + ", name=" + name + ", age=" + age 
				+ ", jno=" + jno + ", jname = " + getJname()
				+ ", info=" + info + ", point=" + point
				+ ", captain = " + getCaptain() + ", cnam = " + cname 
				+ ", project = " + getProject() + "]";
	}
