...
2. null(값의 존재) 확인 : empty, ==
	=> 쿼리스트링으로 비교 Test
		~/ex03_parameter.jsp?id=admin&password=1234
		~/ex03_parameter.jsp?id=admin&password=
		~/ex03_parameter.jsp?id=admin
		
	2.1) ==null
	=> 해당하는 Parameter가 존재하지않으면 true, 존재하지만 값이 없는 경우 false 
	...
	***null이 아니라 empty로 비교해라***

3. pageContext
	=> Jsp 페이지에 대한 정보를 저장하고 있는 객체. (pageScope)
	=> 기본객체를 return 하는 메서드를 제공.
	=> 유일하게 EL태그 내에서 쓸 수 있음. / 다른 기본 객체를 return해줌.
