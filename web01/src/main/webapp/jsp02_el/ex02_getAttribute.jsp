...
1) 동일한 속성(Attribute)명을 모든 영역에 정의
	...
	=> 동일한 속성명을 사용한 경우에 모두 구별해서 출력하려면
		속성명을 붙여서 적용 (앞에 영역_Scope 객체가 생략된 것임. )
	=> 코드가 길어지더라도 Scope영역을 붙여주면 빨리 찾을 수 있음. 효과적임.
...
2) 서로 다른 속성(Attribute)명을 모든 영역에 정의 
	=> 속성명만 사용하여 출력가능
	=> 그러나 영역(Scope)을 붙여주는 것이 효율적 (direct 로 인식)
	\${pname} => ${pname}
	\${rname} => ${rname}
	\${sname} => ${sname}
	\${aname} => ${aname}
...
3) 연산자 Test
	...
	=> 쿼리스트링으로 Test : ~web01/jsp02_el/ex02_getAttribute.jsp?num1=200&num2=789
	<%
