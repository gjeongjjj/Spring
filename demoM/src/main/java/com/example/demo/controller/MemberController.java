...
public class MemberController {
	...
	//          -> mappingName 에 "check"가 포함되어 있으면 service를 아래의 메서드로 처리하도록
	// 			   service.mCheckList(cri), mCheckRowsCont(cri)
	...
	public String axmcri(HttpServletRequest request, Model model, SearchCriteria cri, PageMaker pageMaker) {
		// 1) Criteria 처리
		// => currPage, rowsPerPage 값들은 Parameter로 전달되어 자동으로 cri 에 set
...
//		String password = request.getParameter("Password");
	...
	public String join(HttpServletRequest request, Model model, MemberDTO dto) throws IOException {
		...
		if (!realPath.contains("-tomcat-")) // 개발중
			...
			realPath = "C:\\jgj\\IDESet\\apache-tomcat-9.0.85\\webapps\\demoM\\resources\\uploadImages\\";
		...
		if (!f1.isFile()) { // 존재하지않는 경우(파일존재의 여부를 확인)
			String basicImagePath = "C:\\jgj\\StudyS\\Spring\\demoM\\src\\main\\webapp\\resources\\images\\basicman4.png";
	...
	public String update(HttpServletRequest request, HttpSession session, Model model, MemberDTO dto) throws IOException {
		...
		if (uploadfilef != null && !uploadfilef.isEmpty()) {
			...
			if (realPath.contains(".eclipse.")) // 개발중. 배포X
				realPath = "C:\\jgj\\StudyS\\Spring\\demoM\\src\\main\\webapp\\resources\\uploadImages\\";
			else // 안개발중
				realPath = "C:\\jgj\\IDESet\\apache-tomcat-9.0.85\\webapps\\demoM\\resources\\uploadImages\\";
