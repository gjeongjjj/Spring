...
function rsLogin() {
	...
	fetch(url, {method: 'Post', 
		...
		// .text() response 다음에 status만 받은 상태니까 바디 리딩을 해줘야 한다. body를 리딩하는 메서드 중에 하나다. 
		...
		if (err.message == '502') alert ('id 또는 password 오류 입니다. 다시 하세요. ');
		else alert (`System 오류 입니다! 잠시후 다시 하세요. status=> ${err.message} `);
	});
...
function axiLoginjj() {
	let url = "/rest/rsLoginjj";
	// json으로 받고 json으로 보낼거기 때문에 메서드 필요 없다. 그냥 rsloginjj 로 보내면 된다. 
