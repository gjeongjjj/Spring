...
//	Criteria cri; //ver01
	...
	public void calcData() {
		// 3.1) spageNo, epageNo
		// => currPage 가 PageBlock 의 spageNo, epageNo 계산
		...
		this.spageNo = epageNo - displayPageNo + 1;
		// => 요청받은 currPage = 11 인 경우라면 -> (int)Math.ceil(11/3) * 3
		//    this.spageNo = 12 - 3 + 1 = 10 -> 결론은 11은 10, 11, 12 그룹에 속함.
		
		// 3.2) lastPageNo 계산 & epageNo 의 적합성  
		// this.totalRowsCount / cri.getRowsPerPage() + 1 ==>> ceil
		...
		// 3.3) prev, next
		prev = this.spageNo==1 ? false : true; // spageNo는 1, 4, 7, ... 그니까 첫번째가 아니라면. true다. 
